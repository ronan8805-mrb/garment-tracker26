...
import { Building2, Plus, Search, Shirt, Edit2 } from "lucide-react";
...
export default function FactoriesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FactoryFormData) => {
      return apiRequest("POST", "/api/factories", data);
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The factory has been created successfully.",
      });
    },
    onError: (error: Error) => {
      toast({
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-4">
          <Button type="submit" disabled={isLoading} data-testid="button-save-factory">
            {isLoading ? "Saving..." : isEdit ? "Update Factory" : "Create Factory"}
          </Button>
        </div>
      </form>
    </Form>
